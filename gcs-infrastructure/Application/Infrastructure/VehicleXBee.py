...
def StartVehicleXBee(PORT: str):
    try:
        xbee = XBee(PORT, BAUD_RATE)  # Opens automatically, no .open() needed
        print(f"Vehicle XBee successfully opened on {PORT}")
    except Exception as e:
        print(f"CRITICAL ERROR: Could not open Vehicle XBee on {PORT}. {e}")
        return
...
def RunCommandThread(xbee: XBee, CommandStopEvent: threading.Event):
    while xbee is not None and not CommandStopEvent.is_set():
        try:
            Data = xbee.retrieve_data()
            if Data:
                CommandInstance = None
                match(Data.received_data[0]):
                    case 1: CommandInstance = Heartbeat.DecodePacket(Data.received_data)
                    case 2: CommandInstance = EmergencyStop.DecodePacket(Data.received_data)
                    case 3: CommandInstance = KeepIn.DecodePacket(Data.received_data)
                    case 4: CommandInstance = KeepOut.DecodePacket(Data.received_data)
                    case 5: CommandInstance = PatientLocation.DecodePacket(Data.received_data)
                    case 6: CommandInstance = SearchArea.DecodePacket(Data.received_data)
                    case _: print("\nRetrieved data:", Data.received_data.decode("utf-8"))
...
def RunTelemetryThread(xbee: XBee, TelemetryStopEvent: threading.Event):
    while xbee is not None and not TelemetryStopEvent.is_set():
        try:
            TelemetryInstance = TelemetryQueue.get(timeout=1)
            if isinstance(TelemetryInstance, Telemetry):
                EncodedTelemetry = TelemetryInstance.Encode()
                xbee.transmit_data(EncodedTelemetry, PacketLibrary.GetGCSMACAddress())
                TelemetryQueue.task_done()
